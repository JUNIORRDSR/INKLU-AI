...
from .busqueda_trabajos_discapacidad import main as buscador
...
- coversador: para mantener conversaciones casuales
- buscador: para busar información actualizada mas alla del año 2024
...
Solo puedo ayudarte hablando o buscando informacion en linea." y terminar la conversación.
...
async def procesar_solicitud(mensaje: str) -> dict:
    ...
    try:
       # Asegúrate de convertir el input a un objeto HumanMessage
        ...
        # Impresion de los mensajes para depuración
        final_message = None

        for step in agente_controlador.stream(
            {"messages": input_message},  # Usa el mensaje formateado correctamente
            stream_mode="values",
        ):
            # Imprime el mensaje completo para depuración   
            step["messages"][-1].pretty_print()

        # Mostrar solo el contenido final
        if final_message:
